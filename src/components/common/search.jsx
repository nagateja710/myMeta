...
const SEARCH_CONFIG = {
  ...
  movies: {
    ...
    api: (q) =>`https://api.themoviedb.org/3/search/movie?query=${encodeURIComponent(q)}&api_key=${process.env.NEXT_PUBLIC_TMDB_KEY}`,
  ...
  games: {
    ...
    api: (q) =>
  `https://api.rawg.io/api/games?search=${encodeURIComponent(q)}&key=${process.env.NEXT_PUBLIC_RAWG_KEY}`,
...
export default function Search() {
  ...
  // ⛔ render nothing visually, but hooks still ran
    ...
    <div ref={containerRef} className="relative w-full">
      ...
      {open && (
        <div className="absolute left-0 right-0 z-50">
          <SearchOverlay loading={loading} results={results} />
        </div>
      )}
    </div>
  );
